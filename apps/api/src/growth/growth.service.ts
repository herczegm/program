...
export class GrowthService {
  ...
  WITH
  users AS (
    SELECT UNNEST(ARRAY[${Prisma.join(userIds)}]::text[]) AS "userId"
  ),
  comps AS (
    SELECT UNNEST(ARRAY[${Prisma.join(competencyIds)}]::text[]) AS "competencyId"
  ),
  pairs AS (
  ...
  start_level AS (
    ...
    WHERE
      clc."targetUserId" IN (${Prisma.join(userIds)})
      AND clc."competencyId" IN (${Prisma.join(competencyIds)})
